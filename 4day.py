...
def transform_to_digits(sections_representation):
    left_limit, right_limit = sections_representation.split('-')
    return [num for num in range(int(left_limit), int(right_limit) + 1)]


def is_pair_has_one_range_fully_contain_the_other(first_elf_data, second_elf_data):
    first_elf_sections_set = set(transform_to_digits(first_elf_data))
    second_elf_sections_set = set(transform_to_digits(second_elf_data))
    return first_elf_sections_set.issubset(second_elf_sections_set) or \
        first_elf_sections_set.issuperset(second_elf_sections_set)


def is_pair_has_overlap(first_elf_data, second_elf_data):
    intersection = set(transform_to_digits(first_elf_data)).intersection(transform_to_digits(second_elf_data))
    return len(intersection) > 0


def find_assignment_pairs(pairs):
    pairs_counter = 0
    for pair in pairs:
        if is_pair_has_one_range_fully_contain_the_other(*pair.split(',')):
            pairs_counter += 1
...
def find_overlapping_pairs(pairs):
    ...
    for pair in pairs:
        if is_pair_has_overlap(*pair.strip().split(',')):
            pairs_with_overlap += 1
    return pairs_with_overlap


if __name__ == "__main__":
    with open("input-4day.txt", mode="r") as file:
        file_data = file.read().splitlines()
    assignments_pairs_counter = find_assignment_pairs(file_data)
    overlapping_pairs_counter = find_overlapping_pairs(file_data)

    print(
        f"\n<<<<---- assignments pairs are {assignments_pairs_counter}"
        f"\n<<<<---- overlapping pairs are {overlapping_pairs_counter}"
    )
