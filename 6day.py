...
def is_marker_detected(variant: list) -> bool:
    for elem in variant:
        if variant.count(elem) > 1:
            return False
    return True


def process_datastream(datastream: str, marker_length: int) -> int:
    pointer = []
    for num, signal in enumerate(datastream):
        if num < marker_length:
            pointer.append(signal)
        else:
            if is_marker_detected(pointer):
                return num
            pointer.append(signal)
            pointer.pop(0)
    raise Exception("Datastream does not contain Elf signal marker!")


if __name__ == "__main__":
    with open(file='input-6day.txt', mode='r') as file:
        data: str = file.read().strip()

    start_of_packet_marker_length = 4
    start_of_packet_nums = process_datastream(data, marker_length=start_of_packet_marker_length)
    print(
        f"\nHow many characters need to be processed before the first start-of-packet marker is detected? "
        f"- > {start_of_packet_nums} numbers of characters."
    )

    start_of_message_marker_length = 14
    start_of_packet_nums = process_datastream(data, marker_length=start_of_message_marker_length)
    print(
        f"\nHow many characters need to be processed before the first start-of-message marker is detected? "
        f"- > {start_of_packet_nums} numbers of characters."
    )